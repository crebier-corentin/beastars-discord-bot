...
export function executeCommand(msg: Message) {
    ...
    try {
        ...
        if (!res.success) {
            //Triple cheeks sebun
            const sebunCheeks = msg.guild?.emojis.cache.find((emoji) => emoji.name == "Sebun_Cheeks");
            const legoshiLick = msg.guild?.emojis.cache.find((emoji) => emoji.name == "Legoshi_Lick");
